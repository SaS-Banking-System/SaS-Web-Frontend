...
async function login() {
  ...
  const companyRequest = await useFetch(
    `http://localhost:3001/company/info/${companyCode.value}`,
  );
...
<template>
  <div class="wrapper">
    ...
    <form @submit.prevent="login">
      ...
      <p class="wrong-ccode" v-if="wrongCompanyCode">
        Der eingegebene Code konnte nicht gefunden werden
      </p>
    </form>
    <a class="index-page-a" href="/"
      ><button class="index-page-button">Zur Startseite</button></a
    >
  </div>
</template>
