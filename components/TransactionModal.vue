...
sanitizedScannerUUID.value =
  new Array(Math.floor(props.sender.length / 2)).fill("*").join("") +
  props.sender.slice(Math.ceil(props.sender.length / 2), props.sender.length);
...
async function handleTransaction() {
  if (!String(amount.value).match(/^[0-9]+$/)) {
    notNumberError.value = true;
    return;
  }

  const transactionRequest = await useFetch(
    "http://localhost:3001/transaction/new",
    {
      method: "post",
      body: {
